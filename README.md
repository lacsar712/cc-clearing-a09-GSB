...
单币种多边轧差清算全栈演示：录入义务 → 执行轧差 → 查看净头寸 → 确认 settle → 回执对账。
...
8. 「回执对账」选择 seed 预置的 EUR 批次：各会员回执净额与系统净头寸全部相符
9. 操作员将某会员回执净额改坏（如 ±1）并保存，该行标红、状态变为「差异」，汇总标签显示差异数；改回原值后恢复「相符」
10. 使用 `viewer` 登录，确认只能浏览、无法执行写操作（回执对账页可看不可录）
