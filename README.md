...
单币种多边轧差清算全栈演示：录入义务 → 执行轧差 → 查看净头寸 → 回执对账 → 确认 settle。
...
8. 「回执对账」页选择轧差批次，按会员录入回执净额并保存；初始 seed 回执与系统净头寸一致，显示「全部一致」
9. 用操作员把任意一条回执净额改坏（与系统净头寸不一致）后保存，该会员行会标红、状态为「差异」并显示差额（回执 − 系统）
10. 使用 `viewer` 登录，确认回执页只能浏览、录入框禁用，写入接口返回 403
